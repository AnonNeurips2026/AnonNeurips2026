...
from nqs_support_core.utils import make_logpsi_U_afun
...
def sparsify(U):
    ...
    # The problem is that when sharding, jax operators to_sparse() is not well defined
    # so we just try to revert to using the numba operator if available.
    # This shows up in some tests. Never used in actual code.
    if (
        distributed.mode() == "sharding"
        and jax.process_count() > 1
        and isinstance(U, DiscreteJaxOperator)
    ):
        if hasattr(U, "to_numba_operator"):
            U = U.to_numba_operator()
        else:
            # Stuff like RX operator do not have a numba operator equivalent. But they
            # also do not really create any problem.
            pass
    return U.to_sparse()
