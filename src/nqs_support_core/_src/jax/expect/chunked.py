...
from nqs_support_core._src.jax._vmap_chunked import apply_chunked
...
def expect_chunked(n_chains, chunk_size, log_pdf, expected_fun, pars, *σ):
    ...
    L̄_σ = statistics(L_σ.reshape((n_chains, -1)) if n_chains else L_σ)
...
def expect_chunked_fwd(n_chains, chunk_size, log_pdf, expected_fun, pars, *σ):
    ...
    N_samples = len(out_shape)
    vec = jnp.ones_like(out_shape) / N_samples

    L_σ, gradL_σ = pb_L(vec)

    L̄_stat = statistics(L_σ.reshape((n_chains, -1)) if n_chains else L_σ)
...
def expect_chunked_bwd(n_chains, chunk_size, log_pdf, expected_fun, residuals, dout):
    ...
    def sum_and_mean(x, y):
        # This function  does not do the operations if the x or y
    ...
    grad_f = jax.tree_util.tree_map(sum_and_mean, grad_term1, gradL_σ)
