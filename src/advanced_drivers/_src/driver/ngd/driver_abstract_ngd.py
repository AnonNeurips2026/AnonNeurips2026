...
from netket.stats import Stats, statistics, mean as distributed_mean
...
from nqs_support_core.distributed import sharding_along_axis
...
class AbstractNGDDriver(AbstractVariationalDriver):
    ...
    _collect_residual_info: bool = struct.field(serialize=False)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            collect_residual_info: Whether to collect residual info (aux_vector, ntk_matrix, dv) for
                computing r_t/r_v metrics. Only effective when use_ntk=True and on_the_fly=True.
        ...
        self.collect_residual_info = collect_residual_info
    ...
    def compute_loss_and_update(self):
        ...
        self._dp, self._old_updates, self.info = self.update_fn(
            ...
            collect_residual_info=self.collect_residual_info,
        )
    ...
    @property
    def collect_residual_info(self) -> bool:
        r"""
        Whether to collect residual info for computing r_t/r_v metrics.

        When True, the following quantities are collected in the `info` attribute:
        - `aux_vector`: The centered auxiliary vector a where delta = O.T @ a
        - `ntk_matrix`: The centered NTK matrix
        - `dv`: The centered local energies (scaled)
        - `N_mc`: Number of Monte Carlo samples

        These can be used to efficiently compute O @ delta = NTK @ a
        for the residual r = O @ delta - epsilon.

        Only effective when use_ntk=True and on_the_fly=True.
        """
        return self._collect_residual_info

    @collect_residual_info.setter
    def collect_residual_info(self, value: bool):
        if not isinstance(value, bool):
            raise TypeError("collect_residual_info must be a boolean")
        self._collect_residual_info = value
...
def _get_samples_and_weights(
    ...
    if importance_sampling_distribution is None:
        ...
        # Create a dummy array for shape inference in sharding
        dummy_weights_shape = jnp.zeros((samples.shape[0],))
        weights = _jnp_ones(
            (samples.shape[0],),
            dtype=log_probabilities_p.dtype,
            sharding=sharding_along_axis(dummy_weights_shape, axis=0),
        )
    else:
...
def _jnp_ones(
    ...
    *,
    ...
    """
    # Check if sharding can be used with out_sharding parameter
    # out_sharding cannot contain Auto or Manual axis types
    use_out_sharding = sharding is not None
    if use_out_sharding:
        # Check if we're using an abstract mesh (Auto/Manual axis types)
        # which are not compatible with out_sharding parameter
        try:
            from jax.sharding import get_abstract_mesh

            abstract_mesh = get_abstract_mesh()
            # If abstract mesh exists and has axis names, we're in Auto/Manual mode
            if (
                abstract_mesh is not None
                and hasattr(abstract_mesh, "axis_names")
                and abstract_mesh.axis_names
            ):
                use_out_sharding = False
        except Exception:
            # If we can't check, fall back to with_sharding_constraint to be safe
            use_out_sharding = False
