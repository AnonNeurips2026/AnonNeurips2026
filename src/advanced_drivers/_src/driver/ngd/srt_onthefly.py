...
def srt_onthefly(
    ...
    *,
    ...
    if distributed.mode() == "sharding":
        ...
        # Output shape depends on mode: [N_mc, N_mc, 2, 2] for complex, [N_mc, N_mc] otherwise
        if mode == "complex":
            out_specs = P("S", None, None, None)
        else:
            out_specs = P("S", None)
    ...
    # Store residual info for computing O @ delta = NTK @ a_centered
    # This allows efficient computation of ||O delta - epsilon||^2 / ||epsilon||^2
    if collect_residual_info:
        info['aux_vector'] = aus_vector  # Centered auxiliary vector (before reshape)
        info['ntk_matrix'] = ntk  # Centered NTK matrix
        info['dv'] = dv  # Centered local energies (scaled)
        info['N_mc'] = N_mc
