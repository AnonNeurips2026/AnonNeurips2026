...
from netket.vqs.mc.kernels import local_value_kernel_jax, local_value_kernel_jax_chunked
...
class VMC_NG(AbstractNGDDriver):
    ...
    def _prepare_derivatives(self) -> DerivativesArgs:
        ...
        return log_prob_p_fun, variables_p, samples, weights, (self._ham,)

    @property
    def _kernel(self) -> KernelFun:
        chunk_size = self.state.chunk_size
        if chunk_size is None:
            kernel = local_value_kernel_jax
        else:
            kernel = nkjax.HashablePartial(
                local_value_kernel_jax_chunked, chunk_size=chunk_size
            )
