...
class CheckpointManager:
    def __init__(self, *args, **kwargs):
        raise RuntimeError(
            "Checkpoint restore support requires the optional netket-checkpoint package."
        )


def _restore_checkpoint(*args, **kwargs):
    raise RuntimeError(
        "Checkpoint restore support requires the optional netket-checkpoint package."
    )
...
class AbstractVariationalDriver(struct.Pytree, mutable=True):
    ...
    def restore_checkpoint(
        self,
        chkptr: str | Path | CheckpointManager,
        step=None,
        *,
        loggers=(),
        callbacks=(),
        ...
        tuple[AbstractLog, ...],
        tuple[AbstractCallback, ...],
    ]:
        """
        Restores the state of the driver from a checkpoint.

        If `loggers` or `callbacks` are provided, they will be restored from the checkpoint
        as well and returned.

        Args:
            chkptr: The checkpoint manager to use for restoring the state, or a string/path pointing
        ...
        Returns:
            A tuple containing the restored loggers, callbacks, and the first step of the training
            run when the checkpoint was started.
        """
        # Support for passing a string or Path directly
        if isinstance(chkptr, (str, Path)):
            chkptr = CheckpointManager(
                chkptr, options=CheckpointManagerOptions(read_only=True)
            )
        elif not isinstance(chkptr, CheckpointManager):
            raise TypeError(
                "chkptr must be a `netket_checkpoint.checkpoint.CheckpointManager` or "
                f"a string/Path pointing to a checkpoint directory, Instead got: {type(chkptr)}"
            )
    ...
    ) -> tuple[tuple[AbstractLog, ...], tuple[AbstractCallback, ...], int]:
