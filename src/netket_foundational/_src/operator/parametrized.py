...
class ParametrizedOperator(AbstractOperator):
    """A class for parametrized operators in the context of quantum computing.

    This class is a subclass of `DiscreteJaxOperator` and is designed to handle
    operators that depend on a set of parameters. It provides methods to get and
    set these parameters, as well as to evaluate the operator with respect to a
    given state.

    Attributes:
        hilbert: The Hilbert space associated with the operator.
